...
export const FaithAuditTool = () => {
    const [input, setInput] = useState('');
    const [loadingState, setLoadingState] = useState(LoadingState.IDLE);
    const [showStats, setShowStats] = useState(false);
    const inputRef = useRef<HTMLInputElement>(null);
    const [checklist, setChecklist] = useState({ app: false, streaming: false, giving: false });

    useEffect(() => {
        if (loadingState === LoadingState.SUCCESS) {
            const t1 = setTimeout(() => setChecklist(prev => ({ ...prev, app: true })), 500);
            const t2 = setTimeout(() => setChecklist(prev => ({ ...prev, streaming: true })), 1000);
            const t3 = setTimeout(() => setChecklist(prev => ({ ...prev, giving: true })), 1500);
            const t4 = setTimeout(() => setShowStats(true), 2000);
            return () => { clearTimeout(t1); clearTimeout(t2); clearTimeout(t3); clearTimeout(t4); };
        } else if (loadingState === LoadingState.LOADING) {
            setChecklist({ app: false, streaming: false, giving: false });
            setShowStats(false);
        }
    }, [loadingState]);
                ...
                {loadingState === LoadingState.ERROR ? (
                    <div className="bg-zinc-50 border border-zinc-200 p-8 rounded-xl text-center space-y-4">
                        ...
                        <div className="space-y-1">
                            ...
                            <p className="text-zinc-500 text-sm max-w-sm mx-auto">
                                We couldn't analyze your ministry site. Please try again.
                            </p>
                        </div>
                        <button
                            onClick={handleAudit}
                    ...
                    <div className="bg-white border border-zinc-200 rounded-xl divide-y divide-zinc-200 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-zinc-200 border-b border-zinc-200 bg-zinc-50/50">
                            <div className="p-10 flex flex-col justify-center space-y-1">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h4 className="font-bold text-xs uppercase tracking-widest text-zinc-500">Ministry Plan</h4>
                                </div>
                                <p className="font-semibold text-lg tracking-tight">Your Congregation App</p>
                            </div>
                            <div className="p-10 flex items-center gap-3 cursor-pointer group select-none hover:bg-zinc-50/80 transition-colors">
                                <div className={cn(
                                    ...
                                    checklist.app ? "bg-green-500 border-green-500 text-white" : "bg-white border-zinc-300 group-hover:border-zinc-400"
                                )}>
                                    {checklist.app && <Check className="w-3 h-3" />}
                                </div>
                                <span className={cn("font-medium text-sm transition-colors", checklist.app ? "text-black" : "text-zinc-500")}>
                            ...
                            <div className="p-10 flex items-center gap-3 cursor-pointer group select-none hover:bg-zinc-50/80 transition-colors">
                                <div className={cn(
                                    "w-5 h-5 rounded-md border flex items-center justify-center transition-all shrink-0",
                                    checklist.streaming ? "bg-green-500 border-green-500 text-white" : "bg-white border-zinc-300 group-hover:border-zinc-400"
                                )}>
                                    {checklist.streaming && <Check className="w-3 h-3" />}
                                </div>
                                <span className={cn("font-medium text-sm transition-colors", checklist.streaming ? "text-black" : "text-zinc-500")}>
                                    Sermon Streaming & Devotionals
                                </span>
                            </div>
                            <div className="p-10 flex items-center gap-3 cursor-pointer group select-none hover:bg-zinc-50/80 transition-colors">
                                <div className={cn(
                                    "w-5 h-5 rounded-md border flex items-center justify-center transition-all shrink-0",
                                    checklist.giving ? "bg-green-500 border-green-500 text-white" : "bg-white border-zinc-300 group-hover:border-zinc-400"
                                )}>
                                    {checklist.giving && <Check className="w-3 h-3" />}
                                </div>
                                <span className={cn("font-medium text-sm transition-colors", checklist.giving ? "text-black" : "text-zinc-500")}>
                                    In-App Tithes & Giving
                                </span>
                            </div>
                        </div>
                        ...
                        <div className={cn(
                            ...
                            <StatCard label="Daily Engagement" value={checklist.app ? "+65%" : "0%"} sub="Beyond Sunday Services" icon={TrendingUp} loading={loadingState === LoadingState.LOADING} />
                            <StatCard label="Member Connection" value={checklist.streaming ? "7x" : "1x"} sub="Weekly Touchpoints" icon={Users} loading={loadingState === LoadingState.LOADING} />
                            <StatCard label="Digital Giving" value={checklist.giving ? "+32%" : "0%"} sub="Increase in Offerings" icon={BarChart3} loading={loadingState === LoadingState.LOADING} />
                            <StatCard label="Community" value={checklist.app ? "+89%" : "0%"} sub="Small Group Participation" icon={Heart} loading={loadingState === LoadingState.LOADING} />
                        </div>
                        ...
                        <div className={cn(
                            ...
                            <div className="space-y-3">
                                ...
                                <h3 className="text-2xl md:text-3xl font-bold tracking-tight text-black max-w-xl leading-tight">
                                    Your congregation is waiting.<br />Meet them where they are.
                                </h3>
                                <p className="text-zinc-500 text-lg">$650/mo. Sermons, giving, devotionals, groups. All included.</p>
                            </div>
